...
from eq_layer import variance_scaling_lr
regul = 0.0001
...
class protein_model:
    ...
    def addNoise(self, d, doTrain):
        ...
        if doTrain: d = d * tf.exp(tf.truncated_normal(tf.shape(d), mean=0.0, stddev=0.7))
    ...
    def createResult(self, in_batch, type_batch, dims_batch, gene_id, reuse = False, doTrain = True):
        ...
        else: type_hot = tf.zeros([tf.shape(dims_batch)[0],1]) #do not use type if testset_by_type
        ...
        in_f = self.dense(dims_batch, units=80, name="middleF", reuse=reuse, kernel_regularizer=tf.contrib.layers.l2_regularizer(regul), initZero=False)
